  ...
  <div class="panel-body">
    <div class="row">
      <div class="col-sm-4">
        ...
        <div class="form-group form-group-sm">
          <?= form_label('Kilometer', 'kilometer', ['class' => 'control-label', 'style' => 'width:100%;']) ?>
          <?= form_input([
            'name' => 'kilometer',
            'id' => 'kilometer',
            'class' => 'form-control input-sm',
        ...
        <div class="form-group form-group-sm">
          <?= form_label('Next Visit', 'next_visit', ['class' => 'control-label', 'style' => 'width:100%;']) ?>
          <?= form_input([
            'type' => 'date',
            'name' => 'next_visit',
            'id' => 'next_visit',
      ...
      <div class="col-sm-4">
        ...
        <div class="form-group form-group-sm">
          <?= form_label('Avg. Oil KM', 'avg_oil_km', ['class' => 'control-label', 'style' => 'width:100%;']) ?>
          <?= form_input([
            'name' => 'avg_oil_km',
            'id' => 'avg_oil_km',
            'class' => 'form-control input-sm',
      ...
      <div class="col-sm-4">
        ...
        <div class="form-group form-group-sm">
          <?= form_label('Avg KM / Day', 'avg_km_day', ['class' => 'control-label', 'style' => 'width:100%;']) ?>
          <?= form_input([
            'name' => 'avg_km_day',
            'id' => 'avg_km_day',
            'class' => 'form-control input-sm',
        ...
        <div class="form-group form-group-sm">
          <label class="control-label" style="width:100%;font-weight:bold;">Calculated Avg. Per Day</label>
          <span id="calculated_avg_per_day" style="color:#d9534f; font-size:18px; font-weight:bold; display:block; padding-top:7px;"></span>
        </div>
      </div>
    </div>
  </div>
...
<script type="application/javascript">
  ...
  $(document).ready(function() {
    ...
    // Save vehicle data when completing sale
    $('#finish_sale_button').click(function(e) {
      e.preventDefault(); // Prevent immediate form submission
      saveVehicleDataAndComplete();
    });
    ...
    // Function to save vehicle data and complete sale
    function saveVehicleDataAndComplete() {
      var vehicleData = {
        vehicle_no: $('#vehicle_no').val(),
        kilometer: $('#kilometer').val(),
        last_avg_oil_km: $('#avg_oil_km').val(),
        last_avg_km_day: $('#avg_km_day').val(),
        last_next_visit: $('#next_visit').val(),
      ...
      if (vehicleData.vehicle_no) {
        $.ajax({
          ...
          success: function(response) {
            // After vehicle data is saved, complete the sale
            $('#buttons_form').attr('action', "<?= "$controller_name/complete" ?>");
            $('#buttons_form').submit();
          },
          error: function() {
            // Even if vehicle save fails, still complete the sale
            $.notify({
        ...
        // No vehicle data to save, just complete the sale
        $('#buttons_form').attr('action', "<?= "$controller_name/complete" ?>");
        $('#buttons_form').submit();
      }
    }

    // Remove the old click handler that was duplicated
    ...
    // Vehicle number autocomplete and lookup
    $('#vehicle_no').autocomplete({
      source: "<?= site_url('vehicles/suggest') ?>",
      minChars: 0,
      delay: 500,
      select: function(a, ui) {
        $(this).val(ui.item.value);
        loadVehicleData(ui.item.value);
        return false;
      }
    });

    // Vehicle number change event
    $('#vehicle_no').blur(function() {
      var vehicle_no = $(this).val();
      if (vehicle_no && vehicle_no.trim() !== '') {
        loadVehicleData(vehicle_no);
      }
    });
    ...
    $('#phone_number').blur(function() {
      ...
      if (phone_number && phone_number.trim() !== '') {
        loadCustomerByPhone(phone_number);
      }
    });

    // Function to load vehicle data
    function loadVehicleData(vehicle_no) {
      $.ajax({
        url: "<?= site_url('vehicles/getByVehicleNo') ?>",
        method: 'GET',
        ...
        success: function(response) {
          if (response.success && response.vehicle) {
            ...
            $('#kilometer').val(response.vehicle.kilometer || '');
            $('#avg_oil_km').val(response.vehicle.last_avg_oil_km || '');
            $('#avg_km_day').val(response.vehicle.last_avg_km_day || '');
            $('#next_visit').val(response.vehicle.last_next_visit || '');
            
            // Calculate and display average per day
            calculateAveragePerDay();
            
            // Load customer if associated
            if (response.vehicle.last_customer_id) {
              loadCustomerById(response.vehicle.last_customer_id);
            }
            ...
            // New vehicle - clear fields but keep vehicle number
            $('#kilometer').val('');
            $('#avg_oil_km').val('');
            $('#avg_km_day').val('');
            $('#next_visit').val('');
            $('#calculated_avg_per_day').text('0');
          }
        },
        error: function() {
          $.notify({
    ...
    // Function to load customer by phone number
    function loadCustomerByPhone(phone_number) {
      $.ajax({
        url: "<?= site_url('customers/byPhoneNumber') ?>",
        method: 'GET',
        data: { phone_number: phone_number },
        dataType: 'json',
        success: function(response) {
          if (response.success && response.customer) {
            // Make POST request to selectCustomer with customer ID
            $.ajax({
              url: "<?= site_url('sales/selectCustomer') ?>",
              method: 'POST',
              data: { customer: response.customer.person_id },
              success: function() {
                // Reload the page to show updated customer info
                location.reload();
              },
              error: function() {
                $.notify({
    ...
    // Function to load customer by ID (from vehicle lookup)
    ...
    // Function to calculate average per day
    function calculateAveragePerDay() {
      var avgOilKm = parseFloat($('#avg_oil_km').val()) || 0;
      var avgKmDay = parseFloat($('#avg_km_day').val()) || 0;
      
      if (avgOilKm > 0 && avgKmDay > 0) {
        var avgPerDay = avgOilKm / avgKmDay;
        $('#calculated_avg_per_day').text(avgPerDay.toFixed(1));
      }
    }

    // Calculate average when fields change
    $('#avg_oil_km, #avg_km_day').on('input', calculateAveragePerDay);
  ...
  document.body.onkeyup = function(e) {
    ...
    switch (event.keyCode) {
      ...
      // Auto-add payment when amount changes (replace button click functionality)
    $('.payment-amount').on('input blur', function() {
      ...
      if (amount > 0) {
        ...
        // Submit the payment automatically
        $('#single_payment_form').submit();
      }
    });
    
    // Enter key handling for payment inputs
    ...
    // Auto-focus on first payment input
    $('#payment_cash').focus();
</script>
