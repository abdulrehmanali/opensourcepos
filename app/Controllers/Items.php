...
class Items extends Secure_Controller
{
    ...
    $upload_pdf = $this->upload_pdf();
		...
		if (!empty($upload_pdf['orig_name']) && $upload_pdf['raw_name']) {
  ...
  /**
	 ...
	 * @return array
	 */
	private function upload_pdf(): array
	{
		$file = $this->request->getFile('items_pdf');
		if(!$file)
		{
			return [];
		}
		...
		if (!$this->validate($validation_rule))
		{
			return (['error' => $this->validator->getError('items_pdf')]);
		}
