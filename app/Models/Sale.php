...
class Sale extends Model
{
  ...
  public function get_info(int $sale_id): ResultInterface
  {
    ...
    $sql = 'sales.sale_id AS sale_id,
                ...
                MAX(sales.vehicle_id) AS vehicle_id,
                MAX(sales.mechanic_name) AS mechanic_name,
                MAX(sales.vehicle_kilometer) AS vehicle_kilometer,
                MAX(sales.vehicle_avg_oil_km) AS vehicle_avg_oil_km,
                MAX(sales.vehicle_avg_km_day) AS vehicle_avg_km_day,
                MAX(sales.dinner_table_id) AS dinner_table_id,
                MAX(sales.sale_type) AS sale_type,
                MAX(sales.work_order_number) AS work_order_number,
  ...
  public function save_value(
    ...
    ?int $dinner_table_id,
    ?array &$sales_taxes,
    ?int $vehicle_id = null,          // Add this parameter
  ...
  ): int    //TODO: this method returns the sale_id but the override is expecting it to return a bool. The signature needs to be reworked.  Generally when there are more than 3 maybe 4 parameters, there's a good chance that an object needs to be passed rather than so many params.
  {
    ...
    foreach ($payments as $payment_id => $payment) {
      ...
      $total_amount = floatval($total_amount) + floatval($payment['payment_amount']) - floatval($payment['cash_refund'] ?? 0);
    }
    ...
    foreach ($items as $line => $item_data) {
      ...
      // Handle missing price key (might be item_unit_price from database)
      $price = $item_data['price'] ?? $item_data['item_unit_price'] ?? 0;
      if ($price == 0.00) {
      ...
      $sales_items_data = [
        ...
        'quantity_purchased' => $item_data['quantity'] ?? $item_data['quantity_purchased'] ?? 0,
        'discount' => $item_data['discount'] ?? 0,
        'discount_type' => $item_data['discount_type'] ?? 0,
        'item_cost_price' => $item_data['cost_price'] ?? 0,
        'item_unit_price' => $price,
        'item_location' => $item_data['item_location'] ?? 0,
        'print_option' => $item_data['print_option'] ?? 0
      ];
